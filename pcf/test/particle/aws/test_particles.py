...
@pytest.mark.parametrize("definition,updated_definition,test_tag", values, ids=list(testdata.keys()))
def test_apply(definition, updated_definition, test_tag):
    ...
    with ExitStack() as stack:
        if test_tag[0] == "placebo":
            session = boto3.Session()
            dirname = os.path.dirname(__file__)
            filename = os.path.join(dirname, test_tag[1])
            pill = placebo.attach(session, data_path=filename)
            pill.playback()
        else:
            for context in test_tag:
