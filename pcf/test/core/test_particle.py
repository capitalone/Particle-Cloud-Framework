...
class ParticlePassingVars(Particle):
    ...
    def sync_state(self):
        if self.name == "parent":
            self.current_state_definition = {"item":"var_to_be_passed", "item2":"var2_to_be_passed", "nested":{"nested_key":"nested_var"}}
...
def test_passing_vars():
    ...
    assert(quasiparticle.get_particle("particle_flavor_passing_vars","child").particle_definition["parent_list_var"] == ["var_to_be_passed", "var2_to_be_passed"])
